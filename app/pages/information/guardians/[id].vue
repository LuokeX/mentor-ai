  ...
  <div class="mx-auto max-w-6xl px-5 py-10">
    ...
    <section v-if="commSnapshot" class="panel mt-6 p-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <p v-if="commSnapshot?.assessedAt" class="text-xs text-slate-400">最近评估：{{ formatDateTime(commSnapshot.assessedAt) }}</p>
      </div>
        ...
        <div class="rounded-2xl border border-slate-100 p-5">
          ...
          <div class="mt-4 space-y-3 text-sm">
            ...
            <div class="flex flex-wrap items-center gap-2"><span class="w-24 text-slate-500">复评安排</span>
              <span v-if="commPlan?.nextReviewAt" class="text-slate-600">下次复评 {{ formatDate(commPlan.nextReviewAt) }}</span>
              <span v-else class="text-slate-400">—</span>
            </div>
            ...
            <UButton v-if="commPlan" :to="`/plans/${commPlan.id}`" icon="i-lucide-file-text" size="sm" color="neutral" variant="soft">查看方案与复评记录</UButton>
          </div>
        </div>
      </div>
    </section>
