...
const nav = [{id:'overview',label:'平台总览',icon:'i-lucide-layout-dashboard'},{id:'schools',label:'学校管理',icon:'i-lucide-building-2'},{id:'resources',label:'三库运营台',icon:'i-lucide-library',to:'/platform-admin/resources'},{id:'access',label:'应急访问',icon:'i-lucide-key-round'},{id:'audit',label:'操作审计',icon:'i-lucide-scroll-text'}]
...
<template>
  <div class="mx-auto max-w-7xl px-5 py-9">
    <div class="flex flex-wrap items-end justify-between gap-4"><div><p class="text-sm font-semibold text-indigo-700">平台治理空间</p><h1 class="mt-2 text-3xl font-semibold">平台管理后台</h1><p class="mt-2 text-sm text-slate-500">租户、服务状态与受控应急访问。</p></div><div class="flex flex-wrap items-center gap-3"><UBadge color="warning" variant="soft">默认不接触学校业务数据</UBadge></div></div>
