...
<template>
  <div class="mx-auto max-w-7xl px-5 py-9">
        ...
        <template v-if="active==='knowledge'">
          <div class="space-y-5">
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div class="panel p-5"><div class="flex items-center justify-between"><span class="text-sm text-slate-500">知识片段</span><UIcon name="i-lucide-blocks" class="text-violet-600" /></div><strong class="mt-2 block text-3xl">{{ knowledgeStats.chunks }}</strong><span class="mt-1 block text-xs text-slate-400">用于检索与引用</span></div>
            </div>
    ...
    <UModal :open="Boolean(previewDocument)" :title="previewDocument?.title || '文档预览'" description="以下为教师助手实际检索的分块内容预览。" @update:open="value=>{if(!value)previewDocument=null}"><template #body><div v-if="previewDocument" class="space-y-4"><div class="grid grid-cols-3 gap-3"><div class="rounded-xl bg-slate-50 p-3"><span class="text-xs text-slate-400">格式</span><strong class="mt-1 block text-sm uppercase">{{ previewDocument.sourceType }}</strong></div><div class="rounded-xl bg-slate-50 p-3"><span class="text-xs text-slate-400">字符数</span><strong class="mt-1 block text-sm">{{ previewDocument.metadata?.characterCount || 0 }}</strong></div><div class="rounded-xl bg-slate-50 p-3"><span class="text-xs text-slate-400">知识片段</span><strong class="mt-1 block text-sm">{{ previewDocument.metadata?.chunkCount || 0 }}</strong></div></div><div class="max-h-[50vh] space-y-3 overflow-y-auto pr-1"><div v-for="chunk in previewDocument.chunks" :key="chunk.id" class="rounded-2xl border border-slate-100 p-4"><div class="flex items-center justify-between gap-3"><strong class="text-sm">{{ chunk.heading || `片段 ${chunk.chunkIndex+1}` }}</strong><span class="text-xs text-slate-400">约 {{ chunk.tokenEstimate }} tokens</span></div><p class="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-600">{{ chunk.content }}</p></div></div><div class="flex items-center justify-between border-t border-slate-100 pt-4"><span class="text-xs text-slate-400">导入于 {{ formatKnowledgeDate(previewDocument.createdAt) }}</span><UButton color="error" variant="soft" icon="i-lucide-trash-2" :disabled="selectedKnowledgeBase?.status==='published'" :loading="pending" @click="deleteKnowledgeDocument(previewDocument)">删除文档</UButton></div></div></template></UModal>
