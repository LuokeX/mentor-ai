  ...
  <main class="grid min-h-[100dvh] place-items-center bg-[#f8faf6] p-5">
    <section class="panel w-full max-w-lg p-6 sm:p-9">
      ...
      <div v-if="step === 'token'" class="mt-8 space-y-5">
        ...
        <UButton block size="xl" :loading="loading" @click="validateToken">验证邀请</UButton>
      </div>
      ...
      <form v-else-if="step === 'password'" class="mt-8 space-y-5" @submit.prevent="setPassword">
        ...
        <UButton type="submit" block size="xl" :loading="loading">继续</UButton>
      </form>
      ...
      <div v-else-if="step === 'mfa'" class="mt-8 space-y-5 text-center">
        ...
        <UButton block size="xl" :loading="loading" @click="confirmMfa">确认绑定</UButton>
      </div>
      ...
      <div v-else-if="step === 'recovery'" class="mt-8 space-y-5">
        ...
        <UButton block color="neutral" variant="soft" icon="i-lucide-copy" @click="copyRecoveryCodes">复制恢复码</UButton>
        <UButton block size="xl" @click="finishRecovery">我已安全保存</UButton>
      </div>
      ...
      <div v-else class="mt-8 space-y-5 text-center">
        ...
        <UButton to="/login" block size="xl">前往登录</UButton>
      </div>
