...
const nav = [
  { id: 'overview', label: '管理首页', icon: 'i-lucide-layout-dashboard' }, { id: 'users', label: '用户管理', icon: 'i-lucide-users' }, { id: 'departments', label: '部门管理', icon: 'i-lucide-building-2' }, { id: 'classes', label: '班级管理', icon: 'i-lucide-school' }, { id: 'students', label: '学生管理', icon: 'i-lucide-graduation-cap' }, { id: 'imports', label: '导入中心', icon: 'i-lucide-file-up' },
  ...
  <div class="mx-auto max-w-7xl px-5 py-9">
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-[15rem_1fr]">
      ...
      <section class="min-w-0">
            ...
            <div class="grid gap-4 sm:grid-cols-4">
                  ...
                  <table class="w-full text-left text-sm">
                    ...
                    <tbody>
                      <tr v-for="item in managedClasses.rows" :key="item.id" class="border-t border-slate-100">
                        ...
                        <td class="w-44"><p class="text-xs text-slate-500 truncate">{{ item.ownerName }}</p><div class="mt-0.5 flex items-center gap-1"><USelect v-model="assignment[`class:${item.id}`]" :items="schoolInfo?.teachers?.map((t:any)=>({label:t.name,value:t.id})) || []" size="xs" class="w-28" placeholder="更换教师" /><UButton size="xs" color="neutral" variant="soft" :disabled="!assignment[`class:${item.id}`]" :loading="pending" @click="assignRecord('class', item.id)">分配</UButton></div></td>
              ...
              <div class="mt-6 rounded-2xl border border-slate-100 p-5">
                ...
                <div v-if="showClassHistory" class="mt-4 divide-y divide-slate-100">
                  <div v-for="item in schoolInfo?.assignments" :key="item.id" class="grid gap-2 py-3 text-xs text-slate-500 md:grid-cols-[.6fr_1fr_1fr]"><span>{{ item.targetType }} · {{ item.fromUserName }} → {{ item.toUserName }}</span><span>{{ item.reason || '未填写原因' }}</span><span class="md:text-right">{{ new Date(item.createdAt).toLocaleString('zh-CN') }}</span></div>
        ...
        <template v-if="(active as string) === 'crises'"><div class="panel p-6"><h2 class="text-xl font-semibold">危机转介调度</h2><p class="mt-2 text-sm text-slate-500">按事件严重程度和 SLA 排序。展示教师、来源和时间，便于快速识别。未确认工单可转派。</p><div class="mt-5 divide-y divide-slate-100"><div v-for="item in referrals" :key="item.id" class="grid gap-4 py-4 text-sm lg:grid-cols-[1.2fr_.8fr_.9fr_auto]"><div class="min-w-0"><div class="flex items-center gap-2"><strong class="truncate">{{ item.teacherName || '未知教师' }}</strong><UBadge size="xs" :color="item.severity==='critical'?'error':item.severity==='high'?'warning':item.severity==='medium'?'primary':'neutral'" variant="soft">{{ item.severity || '—' }}</UBadge></div><p class="mt-1 text-xs text-slate-500">事件 {{ item.safetyEventId.slice(0,8) }}<span v-if="item.sourceType" class="text-slate-400"> · {{ item.sourceType }}</span><span v-if="item.matchedRules?.length" class="text-slate-400"> · {{ item.matchedRules.slice(0,2).join(', ') }}{{ item.matchedRules.length > 2 ? '...' : '' }}</span></p><p class="mt-0.5 text-xs text-slate-400">{{ item.eventCreatedAt ? new Date(item.eventCreatedAt).toLocaleString('zh-CN') : '—' }}</p></div><div><div class="flex flex-wrap items-center gap-1.5"><UBadge size="xs" :color="item.status==='created'?'error':item.status==='escalated'?'warning':item.status==='acknowledged'?'primary':item.status==='closed'?'neutral':'neutral'" variant="soft">{{ item.status==='created'?'待确认':item.status==='escalated'?'已升级':item.status==='acknowledged'?'处理中':item.status==='closed'?'已关闭':item.status }}</UBadge><UBadge size="xs" :color="item.priority==='critical'?'error':item.priority==='urgent'?'warning':'neutral'" variant="soft">{{ item.priority }}</UBadge></div><p v-if="item.acknowledgeDueAt && !item.acknowledgedAt" class="mt-1.5 text-xs font-medium" :class="getSlaClass(item.acknowledgeDueAt)">{{ getSlaRemaining(item.acknowledgeDueAt) }} · 截止 {{ new Date(item.acknowledgeDueAt).toLocaleTimeString('zh-CN') }}</p><p v-else-if="item.acknowledgedAt" class="mt-1.5 text-xs text-slate-400">已确认 {{ new Date(item.acknowledgedAt).toLocaleString('zh-CN') }}</p><p v-else class="mt-1.5 text-xs text-slate-400">无 SLA 截止</p></div><div><p v-if="item.psychologistId" class="mb-1.5 text-xs text-slate-500">当前: {{ getPsychologistName(item.psychologistId) }}</p><p v-else class="mb-1.5 text-xs text-amber-600 font-medium">未分配</p><USelect v-model="referralAssignee[item.id]" :items="settings?.psychologists?.map((p:any)=>({label:p.name,value:p.id})) || []" :disabled="Boolean(item.acknowledgedAt)||item.status==='closed'" placeholder="转派心理专员" class="w-full" size="xs" /></div><UButton size="xs" :disabled="!referralAssignee[item.id] || Boolean(item.acknowledgedAt) || item.status==='closed'" @click="reassignReferral(item)">转派</UButton></div><p v-if="!referrals?.length" class="py-14 text-center text-sm text-slate-400">暂无危机转介</p></div></div></template>
