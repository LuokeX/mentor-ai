...
const { rows, total, page, pageSize, q, sort, order, loading, error, onSearch, onSortChange, onPageChange, onPageSizeChange, refresh } = useManagedList<{ id: string; action: string; targetType: string; actorId: string; result: string; createdAt: string }>('/api/v1/school-admin/audit-logs')
const columns = [{ key: 'action', label: '操作' }, { key: 'targetType', label: '目标类型' }, { key: 'actorId', label: '操作人' }, { key: 'result', label: '结果' }, { key: 'createdAt', label: '时间' }]
</script>
<template>
  <ManagementPage title="审计日志" description="查看操作审计记录">
