...
const { rows, total, page, pageSize, q, statusFilter, sort, order, loading, error, pageCapabilities, onSearch, onStatusChange, onSortChange, onPageChange, onPageSizeChange, refresh } = useManagedList<{ id: string; relation: string; status: string; updatedAt: string }>('/api/v1/school-admin/guardians')
...
<template>
  <ManagementPage title="家长管理" description="管理全校家长档案" :can-create="pageCapabilities.includes('create')" create-label="添加家长">
    <TableToolbar :search-value="q" :status-filter="statusFilter" :status-options="statusOpts" search-placeholder="搜索家长..." :loading="loading" @search="onSearch" @update:status-filter="onStatusChange" @refresh="refresh" />
