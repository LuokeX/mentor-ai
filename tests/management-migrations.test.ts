...
describe('management migration safety', () => {
  it('backfills relationship school ownership before enforcing not-null', () => {
    ...
    const addNullable = sql.indexOf('ADD COLUMN "school_id" uuid;')
  ...
  it('adds lifecycle columns and relationship status in a follow-up migration', () => {
    ...
    expect(sql).toContain('ALTER TABLE "departments" ADD COLUMN "archived_at"')
    expect(sql).toContain('ALTER TABLE "chat_sessions" ADD COLUMN "archived_at"')
    expect(sql).toContain('ALTER TABLE "student_events" ADD COLUMN "archived_at"')
    expect(sql).toContain('ALTER TABLE "department_members" ADD COLUMN "status"')
