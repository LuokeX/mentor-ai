...
- 所有消息统一走 Agent 图（`server/agent/graph.ts`），没有 Agent 开关。入口只做认证、装配与 SSE 转发，图执行、回答校验与落库在共享流水线 `server/domain/chat-stream.ts`（普通提问与「重新生成」复用同一份）。Agent 本轮无产出时自动整轮重试一次，重试轮追加一条不落库的临时提示（传输层另有 SDK 自动重试）；重试仍失败由入口发 `error` 事件，不生成兜底回答、不回退其它提示词。
...
- 模型调用（含每次工具往返与摘要调用）写 `ai_model_calls`：只记元数据与输入/输出/缓存命中 token，不记 Prompt 与正文；AI 中心「调用审计」按缓存命中列观察命中率，概览页按 `assistant_tool_called` 产品事件展示工具使用。
