...
- 只读工具共 13 个，按上下文裁剪（`server/agent/tools/index.ts`），数据读取集中在 `server/domain/assistant-readers.ts`：知识检索、模块分诊（含模块 playbook）、量表推荐（与模块页同一套门禁，被锁改推前置；对象级量表——`per_case`、红线检查，以及班级系统这类评估对象是班级/学生/家长的模块——的完成状态与触发条件只认当前会话绑定的同一咨询对象，未绑定对象时按「未对该对象做过」处理，触发依据给的是实测均分与结论等级而不是规则原文）、实体记忆（跨会话读取同一咨询对象的最近 8 条，未传参数时用当前会话绑定的对象）、咨询对象档案、学生检索/档案、方案与复盘、评估历史、沟通记录、班级概览、教师待办、三库资源目录。工具启用清单与轮次上限来自 `AI_AGENT_ENABLED_TOOLS` / `AI_AGENT_MAX_TOOL_ROUNDS`。同轮内相同 (工具, 参数) 的重复调用复用结果，单次工具执行默认 10 秒超时并按失败回传模型自愈。**新增数据能力一律走只读工具**：写操作仍由既有 REST 路由执行，不新增写工具。
