...
### 5.1 代码闸门（单人 trunk 工作流）

单人开发默认在 `main` 上直接进行，核心纪律是「验证通过才 push」：

- 日常直接在本地 `main` 开发，本地 `main` 允许领先远端；3301 本地验证（typecheck/test）后，用当前工作区代码在 3400 完成第 7 节的测试环境演练。
- 演练通过后再 `git commit` 并 `git push origin main`——push 动作即「已验证」声明；正式发布按第 7 节执行。
- 演练未通过的代码不推送、不部署；发现问题回到本地修复后重跑演练。
