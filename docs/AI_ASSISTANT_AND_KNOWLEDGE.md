...
- 外发脱敏与本地模式：`redactOutboundText` 对所有外发文本（提问、历史、轨迹、档案、画像、摘要）按学校数据模式脱敏；`local` 模式直接不调用外部模型并向教师返回提示。姓名脱敏（`server/integrations/deepseek.ts` 的 `redactPii`）只替换「看起来像姓名」的片段（单姓/复姓/小名前缀/叠字小名 + 老师·同学·妈妈·爸爸·家长），不再把「两位家长」「安排家长会」「温柔的老师」这类普通说法一起替换——旧口径会把约三分之一的教师消息改成残缺中文，模型还会把 `[PERSON]` 复读进回答；同时 `server/agent/answer-guard.ts` 在落库前清理任何残留的 `[PERSON]`/`[PHONE]`/`[EMAIL]` 占位符（命中记 `placeholder_leak` 违规，用于观测）。新增姓氏或常见词进这两张表时，请同步 `tests/ai-outbound-redaction.test.ts` 的回归用例。
