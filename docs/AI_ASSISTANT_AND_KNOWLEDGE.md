...
- **本地硬规则命中（明确措辞）**：停答，先创建风险事件、心理专员工单、通知 Outbox 和审计记录，教师端进入转介指引。
- **语义命中（模型判定，首轮识别后由复核提示词逐条确认）**：照常回答，只在回答末尾补一句中性提示（`SAFETY_FOLLOW_UP_NOTE`，不得含教师界面禁用字样）、在消息 metadata 标 `safetyAlert`；后台同样创建风险事件、转介工单与通知。复核调用失败（超时、HTTP、解析）时保留首轮判定——安全侧不因技术失败放行。复核清空首轮判定时记 `assistant_semantic_safety_cleared` 产品事件，用于观察误报率。
