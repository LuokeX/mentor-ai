...
量表库回答“评估什么、怎么采集”。每个模块至少维护一个可发布量表版本。业务侧统一填写 `docs/business/library-standards/三库整理总模板.xlsx`，系统导入时转换为发布数据，运行时发布到 `module_resource_libraries.library_type = assessment`。
