...
export default defineEventHandler(() => ({
  ...
  paths: {
    ...
    '/api/v1/chat/messages': { post: { summary: '安全检查、对象上下文、知识检索、流式 AI 回答、引用与模块建议（SSE）', responses: { 200: { description: 'SSE events: ack(context), answer_start, answer_delta, answer, sources, route, fuse, done' }, 409: { description: 'Session context cannot be switched silently' } } } },
