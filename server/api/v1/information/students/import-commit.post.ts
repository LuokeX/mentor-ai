...
import { commitStudentImport } from '../../../../domain/information-import-io'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await commitStudentImport(event, { userId: user.id, schoolId: user.schoolId, contentBase64: body.contentBase64 })
  if (!result.ok) {
