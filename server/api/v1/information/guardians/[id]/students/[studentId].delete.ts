...
export default defineEventHandler(async (event) => {
  ...
  const [guardian] = await db.select({ id: schema.guardians.id }).from(schema.guardians)
    .where(and(eq(schema.guardians.id, id), eq(schema.guardians.ownerUserId, user.id), eq(schema.guardians.schoolId, user.schoolId!))).limit(1)
  ...
  const [student] = await db.select({ id: schema.students.id }).from(schema.students)
    .where(and(eq(schema.students.id, studentId), eq(schema.students.ownerUserId, user.id), eq(schema.students.schoolId, user.schoolId!))).limit(1)
  if (!student) throw createError({ statusCode: 422, message: '学生不存在或不属于当前教师' })

  // 家长-学生关系不物理删除，改为标记为 removed
  ...
  await writeAudit(event, {
    ...
    actorId: user.id,
    action: 'information.guardian.student.unlink',
    targetType: 'guardian',
    targetId: id,
    metadata: { studentId }
  })
