...
export default defineEventHandler(async (event) => {
  ...
  const rows = result.rows.map((row) => {
    ...
    return {
      ...
      _capabilities: resolveCapabilities({ user, recordSchoolId: row.schoolId, recordOwnerUserId: row.ownerUserId, recordStatus: row.status, targetType: 'student_event', targetId: row.id }),
    }
  })
