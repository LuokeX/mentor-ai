...
export default defineEventHandler(async (event) => {
  ...
  const result = await paginateResult({
    dataQuery: db.select({ id: schema.studentEvents.id, eventType: schema.studentEvents.eventType, severity: schema.studentEvents.severity, title: schema.studentEvents.title, studentId: schema.studentEvents.studentId, occurredAt: schema.studentEvents.occurredAt, status: schema.studentEvents.status, ownerUserId: schema.studentEvents.ownerUserId, schoolId: schema.studentEvents.schoolId, createdAt: schema.studentEvents.createdAt, updatedAt: schema.studentEvents.updatedAt }).from(schema.studentEvents).where(and(...conditions)).orderBy(orderFn(sortCol)).limit(query.pageSize).offset(offsetFrom(query.page, query.pageSize)),
    countQuery: db.select({ value: countSql }).from(schema.studentEvents).where(and(...conditions)),
    page: query.page, pageSize: query.pageSize,
  })
  const rows = await Promise.all(result.rows.map(async (row) => {
    const caps = await resolveCapabilities({ user, recordSchoolId: row.schoolId, recordOwnerUserId: row.ownerUserId, recordStatus: row.status, targetType: 'student_event', targetId: row.id })
