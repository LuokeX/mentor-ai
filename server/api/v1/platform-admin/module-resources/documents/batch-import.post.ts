...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    documents = parseKnowledgeSheets(sheets, body.defaultModule as ModuleId)
  } catch (error: any) {
    throw createError({ statusCode: 400, message: error?.message || '文件解析失败' })
  }

  if (!documents.length) throw createError({ statusCode: 400, message: '文件中没有可导入的知识文档' })
  ...
  for (const doc of documents) {
    ...
    try {
      embeddings = await embedModuleResourceChunks(event, texts)
    } catch { /* embedding 失败不阻塞导入 */ }
    ...
    if (embeddings) embeddedChunks += embeddings.length
  }

  if (!prepared.length) throw createError({ statusCode: 400, message: '所有文档均无可导入内容' })
