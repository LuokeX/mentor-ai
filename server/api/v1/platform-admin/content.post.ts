...
const schemaInput = z.discriminatedUnion('action', [
  z.object({ action: z.literal('create'), code: z.string().min(2).max(80), name: z.string().min(2).max(160), version: z.string().min(1).max(40), type: z.enum(['assessment', 'tool', 'sop', 'prompt']), payload: z.record(z.string(), z.unknown()) }),
