...
/** 发布提示词草稿：published = 当前草稿内容，立即热生效（失效进程内缓存）。 */
export default defineEventHandler(async (event) => {
  ...
  const [existing] = await db
    .select({ id: schema.aiPromptTemplates.id, template: schema.aiPromptTemplates.template })
    .from(schema.aiPromptTemplates)
    .where(eq(schema.aiPromptTemplates.code, code))
    .limit(1)
  if (!existing?.template?.trim()) throw createError({ statusCode: 409, message: '请先保存草稿再发布' })
