...
export default defineEventHandler(async (event) => {
  ...
  const [message] = await db.select({ id: schema.chatMessages.id, sessionId: schema.chatMessages.sessionId })
    .from(schema.chatMessages).where(and(
      eq(schema.chatMessages.id, messageId), eq(schema.chatMessages.schoolId, user.schoolId), eq(schema.chatMessages.ownerUserId, user.id), eq(schema.chatMessages.role, 'assistant'),
