...
export default defineEventHandler(async (event) => {
  ...
  const [updated] = await db.update(schema.delegatedManagementGrants).set({
    ...
    updatedAt: now
  }).where(eq(schema.delegatedManagementGrants.id, id)).returning()
  if (!updated) throw createError({ statusCode: 404, message: '代管申请不存在' })
  await writeAudit(event, {
    ...
    metadata: { requesterId: request.requesterId, scopes: request.scopes }
  })
  return updated
})
