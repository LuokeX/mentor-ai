 ...
 * 模型名优先取 AI 管理中心 DB 运行时配置（getAiRuntimeConfig 缓存 30s），
 * 无则回落环境变量默认值。不输出 thinking 参数，与现有 deepseek.ts 行为一致。
 */
 ...
 *
 * 说明：getAiRuntimeConfig 是异步的（DB 运行时配置优先），因此本函数为 async；
...
export async function createAgentLlm(
  ...
  // DB 运行时配置（如 30000/60000）优先；无则回落环境变量 DEEPSEEK_TIMEOUT_MS，仍为空给 45s。
