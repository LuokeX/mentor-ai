...
const SSO_SCOPE = 'openid profile'
...
/** 按优先级匹配本地账号：已绑定 subject → 工号（仅唯一命中）；邮箱不再作为匹配键 */
...
function ssoDenied(message: string) {
...
export async function completeSsoLogin(event: H3Event): Promise<AuthUser> {
  ...
  if (!user) {
    throw ssoDenied('该账号未在本平台开通，请联系学校管理员')
  }
  if (user.status !== 'active') {
    throw ssoDenied('该账号已被停用，请联系学校管理员')
  }
  if (user.schoolId) {
    ...
    if (!school || school.status !== 'active') {
      throw ssoDenied('该账号所属学校不可用，请联系管理员')
    }
  }
