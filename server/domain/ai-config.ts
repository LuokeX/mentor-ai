...
{{businessContextText}}`
  },
  {
    ...
    template: `${SYSTEM_MARKER}你是教师赋能平台的澄清判定器。只返回 JSON，不做诊断，不回答教师问题。
###USER###
...
JSON结构示例：{{jsonFormat}}`
  },
  {
    ...
    name: '工具步骤人话改写提示词',
    description: '把工具库机械的结构化步骤与归因建议/分级干预的一句话建议改写成教师可直接执行的口语化步骤；知识库检索片段（knowledgeChunks）为共同输入，只用于把专业术语解释成教师能懂的白话。tools 非空时工具名与步骤数量/顺序/关键事实不变、只优化表达；tools 为空时仅依据知识片段自拟 1-3 条新工具。actions 逐条按「何时做 → 怎么做 → 话术示例 → 频率/周期 → 达标标准」扩写为可执行步骤，title 与条数必须与输入一致、保留原建议要点，为空时输出空数组。提交链路同步调用，格式校验失败自动重试（最多 3 次），失败回退三库原文。',
    placeholders: [
    ...
    template: `${SYSTEM_MARKER}你是教师赋能平台的工具步骤改写助手。把输入 facts 里的工具步骤与归因建议都改写成「人话版」——教师看了能直接照着执行的口语化内容，像同事手把手交代怎么做，而不是条目清单。facts 含两类需要分别处理的内容：
...
1b. 当 facts.tools 为空时：只能依据 knowledgeChunks 里的知识内容自拟 1-3 条工具步骤，title 简短自拟且不超过 30 字；不得编造知识片段之外的来源、术语或做法。
2. actions 扩写规则：
