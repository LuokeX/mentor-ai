...
JSON结构示例：{{jsonFormat}}`
  },
  {
    ...
    name: '工具步骤人话改写提示词',
    description: '把工具库机械的结构化步骤改写成教师可直接执行的口语化步骤；知识库检索片段（knowledgeChunks）为共同输入，只用于把专业术语解释成教师能懂的白话。tools 非空时工具名与步骤数量/顺序/关键事实不变、只优化表达；tools 为空时仅依据知识片段自拟 1-3 条新工具。提交链路同步调用，格式校验失败自动重试（最多 3 次），失败回退三库原文。',
    placeholders: [
      { key: 'facts', label: '规则事实 JSON', description: '模块、严重度、归因名称/强弱/原因、匹配工具标题与原文内容。' },
    ...
    template: `${SYSTEM_MARKER}你是教师赋能平台的工具步骤改写助手。把工具库机械的结构化步骤改写成「人话版」——教师看了能直接照着执行的口语化内容，像同事手把手交代怎么做，而不是条目清单。工具库的 tools 与知识库检索片段 knowledgeChunks 是共同输入：以 tools 为主体重写正文；knowledgeChunks 只用来把专业术语讲成教师能懂的白话并融入表述（例如把「共情」说成「先说出他现在的感受，让他感觉被理解」），不得当作额外工具或新步骤加入输出。
硬约束：
1. 当 facts.tools 非空时：输出 tools 的数量与 title 必须与输入完全一致，不得新增、删减或改名工具；每个工具内部的步骤数量与先后顺序不得改变；动作要点、提示、话术模板、达标标准等关键事实必须保留，只允许改成更自然的口语表述，不得丢弃、合并或编造事实。
2. 当 facts.tools 为空时：只能依据 knowledgeChunks 里的知识内容自拟 1-3 条工具步骤，title 简短自拟且不超过 30 字；不得编造知识片段之外的来源、术语或做法。
3. 不做精神、医学、法律诊断，不承诺效果，不写"确诊、治疗、治愈、一定、保证、医学诊断"等表述；涉及就医等专业处置只能做一般性提醒，不代替专业意见。
4. 每条工具的输出为口语化段落：步骤之间用换行分隔，可用"你可以先说：…""这一步等对方情绪平稳后再做"等自然引导语；禁止输出"1. 标题: 说明"加"提示：/话术：/达标："式的机械模板，也不要整段照搬知识片段原文。
5. 输出必须是严格 JSON，字段结构与示例完全一致（仅 tools 数组，每项含 title 与 content）。
###USER###
规则事实：{{facts}}
