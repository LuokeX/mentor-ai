...
function inferHeaderRow(rows: unknown[][]) {
  const hints = ['module', 'code', 'instrumentCode', '题号', '题项', '工具编码', '规则编码', '变量名', '优先级', '主归因', '评估编码']
  let bestIndex = 0
  let bestScore = -1
...
function parseToolSheets(sheets: SheetData[]): ToolRxEntry[] {
  return sheets.flatMap(sheet => sheet.rows.map((row): ToolRxEntry | null => {
    ...
    const steps = splitList(read(row, ['steps', '执行步骤', '操作步骤', '操作步骤（详细）', '具体操作步骤', '核心操作']))
    if (!code || !name || !steps.length) return null
    const tool: ToolRxEntry = {
      code,
      name,
      form: read(row, ['form', '工具形式', '处方形式', '工具类型']) || '',
      symptoms: read(row, ['symptoms', '适用症状/场景', '适用问题/场景', '适用场景', '触发情景']) || '',
      steps
    }
    assignIfPresent(tool, 'expectedEffect', read(row, ['expectedEffect', '预期输出或效果', '预期效果', '输出物']))
    assignIfPresent(tool, 'severity', read(row, ['severity', '严重度分级', '风险等级']))
    assignIfPresent(tool, 'level', read(row, ['level', '适用等级', '等级']))
    assignIfPresent(tool, 'attribution', read(row, ['attribution', '适用归因', '主归因']))
    assignIfPresent(tool, 'attributions', splitList(read(row, ['attributions', '归因列表', '适用归因列表'])))
    assignIfPresent(tool, 'primaryAttribution', read(row, ['primaryAttribution', '主归因']))
    ...
    assignIfPresent(tool, 'prohibitions', read(row, ['prohibitions', '禁忌条件', '禁止事项']))
...
function parseAttributionSheets(sheets: SheetData[], module: ModuleId) {
  ...
  for (const sheet of sheets) {
    for (const row of sheet.rows) {
      ...
      const level = read(row, ['level', '等级', '输出等级'])
      if (ruleId || primaryAttribution || level) {
        branches.push({
          pri: Number(read(row, ['priority', 'pri', '优先级'])) || branches.length + 1,
          when: read(row, ['when', '条件表达式', '命中条件']) || undefined,
          level: level || '',
          blocked: parseBool(read(row, ['blocked', '是否阻断', '阻断'])),
          ...
          reasons: splitList(read(row, ['reasons', '原因说明', '原因文案'])),
  ...
  return attributionConfigSchema.parse({ module, version, computed, branches, actions, tools })
}
