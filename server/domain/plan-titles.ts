...
/** 直接评估标题：归因描述前三句；无描述时回退到归因关键词连接。 */
function buildDirectAssessmentTitle(input: PlanTitleInput): string {
  const descriptions = (input.attributionDescriptions || [])
    .map(s => s.trim())
    .filter(Boolean)
  if (descriptions.length) {
    const joined = descriptions.join('')
    const sentences = splitSentences(joined, 3)
...
export function buildPlanTitle(input: PlanTitleInput): { title: string, titleFull: string } {
  const titleFull = input.sourceType === 'assistant_dialogue'
    ? [input.objectLabel || undefined, input.questionSummary?.trim() || undefined]
        .filter(Boolean)
