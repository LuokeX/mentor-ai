...
export function buildFormatInstruction(input: { teacherProfileText?: string }): string {
  ...
  const behaviors = [
    ...
    '先检索再回答：除寒暄与"你能提供什么帮助"这类能力询问外，凡是涉及班主任具体做法、平台资源（量表、工具、SOP、制度、流程）的问题，先调用 knowledge_search 检索已发布知识，需要资源清单时再用 resource_lookup 补全；教师问「这类问题通常有哪些原因」「有没有具体做法」时，再用 resource_detail 查归因项与工具卡明细（可按现象关键词筛）。平台正式内容只能基于工具实际返回的内容。无命中时可针对缺失信息改写检索一次，仍未命中说明“平台里暂时没有对应资源”；允许结合教师已提供的事实给具体的通用专业建议，明确它不是平台制度。',
