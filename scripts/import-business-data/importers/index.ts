...
import { parseToolFile } from '../transformers/tool'
...
export async function runImport(task: ImportTask, options: { dryRun?: boolean; publish?: boolean } = {}): Promise<ImportResult> {
  ...
  try {
    ...
    const payload = await loadPayload(task, filePath)
    ...
    if (dryRun) return { task, count, success: true }
    ...
    return { task, count, success: true }
  } catch (err: any) {
    return { task, count: 0, success: false, error: err.message }
  }
}

export async function importAll(options: { dryRun?: boolean; publish?: boolean; module?: string; type?: string } = {}) {
  let tasks = ALL_TASKS
  ...
  console.log(`\nDone: ${succeeded}/${results.length} tasks, ${totalEntries} total entries`)
...
async function loadPayload(task: ImportTask, filePath: string) {
  if (task.type === 'tool') return { tools: parseToolFile(filePath, task.toolFormat!) }
