...
if (process.env.EMBEDDING_ENABLED !== 'true') {
  process.stdout.write('Embedding is disabled; skipping knowledge reindex\n')
  process.exit(0)
}
...
try {
  while (true) {
    ...
    process.stdout.write(`Indexed ${indexed} knowledge chunks\n`)
  }
  ...
  await pool.query(`
    ...
    where kd.id = summary.document_id
  `)
  process.stdout.write(`Knowledge embedding reindex completed: ${indexed} chunks updated\n`)
} finally {
  await pool.end()
}
